...
function formatDate(isoStr) {
  if (!isoStr) return '';
  let s = isoStr;
  // Handle YYYYMMDD (e.g. "20231205")
  if (/^\d{8}$/.test(s)) s = `${s.slice(0,4)}-${s.slice(4,6)}-${s.slice(6)}`;
  // If already has a time component (T or space), parse as-is; otherwise append to avoid UTC shift
  ...
  listEl.innerHTML = patents.map((p, i) => {
    ...
    return `
      <li class="patent-item"
        ...
        <span class="patent-date">${date}</span>
      </li>`;
  }).join('');
}
